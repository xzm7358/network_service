...
std::string wpa_events_payload(const WpaEventsView &view) {
    std::ostringstream os;
    os << "{"
       << "\"attached\":" << (view.attached ? "true" : "false") << ","
       << "\"connected\":" << (view.connected ? "true" : "false") << ","
       << "\"disconnected\":" << (view.disconnected ? "true" : "false") << ","
       << "\"dhcp_requested\":" << (view.dhcp_requested ? "true" : "false") << ","
       << "\"has_ip\":" << (view.has_ip ? "true" : "false") << ","
       << "\"has_default_route\":" << (view.has_default_route ? "true" : "false") << ","
       << "\"dns_available\":" << (view.dns_available ? "true" : "false") << ","
       << "\"ip4\":\"" << json_escape(view.ip4) << "\","
       << "\"gateway4\":\"" << json_escape(view.gateway4) << "\","
       << "\"dns4\":\"" << json_escape(view.dns4) << "\","
       << "\"connect_events\":" << view.connect_events << ","
       << "\"disconnect_events\":" << view.disconnect_events << ","
       << "\"dhcp_requests\":" << view.dhcp_requests << ","
       << "\"scan_started_events\":" << view.scan_started_events << ","
       << "\"scan_result_events\":" << view.scan_result_events << ","
       << "\"scan_failed_events\":" << view.scan_failed_events << ","
       << "\"event_sequence\":" << view.event_sequence << ","
       << "\"last_scan_started_sequence\":" << view.last_scan_started_sequence << ","
       << "\"last_scan_result_sequence\":" << view.last_scan_result_sequence << ","
       << "\"last_scan_failed_sequence\":" << view.last_scan_failed_sequence << ","
       << "\"wifi_state\":\"" << json_escape(view.wifi_state) << "\","
       << "\"failure_reason\":\"" << json_escape(view.failure_reason) << "\","
       << "\"last_event\":\"" << json_escape(view.last_event) << "\","
       << "\"last_ssid\":\"" << json_escape(view.last_ssid) << "\","
       << "\"last_bssid\":\"" << json_escape(view.last_bssid) << "\""
       << "}";
    return os.str();
}
